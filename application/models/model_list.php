<?php
    class Model_list extends Model{
        ...
        function list_goods($id){
            $result=Model::connect()->query("SELECT * FROM `goods` WHERE id_list='$id'");
            return $result;           
        }
        ...
        function user($email){
            $result=Model::connect()->query("SELECT `id` FROM `users` WHERE email='$email'");
            return $result;
        }
        function user_list_valid($user_id,$list_id){
            $result=Model::connect()->query("SELECT * FROM list_user WHERE user_id='$user_id' and list_id='$list_id'");
            return $result;
            
        }
        function list_user_add($user_id,$list_id){
            $result=Model::connect()-> query("INSERT INTO list_user(user_id,list_id) VALUES('$user_id','$list_id')");
            return $result;
        }
        function list_bloc($id,$date,$bloc_user_id,$bloc=1){
            $result=Model::connect()->query("UPDATE `list`SET `bloc`='$bloc', `last_upd`='$date', `bloc_user_id`='$bloc_user_id' WHERE `id`='$id' ");
        }
